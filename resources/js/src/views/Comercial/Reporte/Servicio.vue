            ...
            <b-row>
                <b-col md="6" xl="8">
                    <b-form-group label="Ruta">
                        <v-select label="nombreRut" v-model="ruta" :options="rutas" style="flex: 1;" />
                    </b-form-group>
                </b-col>
                <b-col md="6" xl="2">
                ...
                <b-col md="12" xl="12">
                    <div class="otabla">
                        <table class="table table-condensed table-bordered">
                            ...
                            <tbody>
                                <tr v-for="(urutas, indexv, nrov) in proyecciones">
                                    <th>{{  nrov+1 }}</th>
                                    <th>{{ urutas.nombrePro }}</th>
                                    <th>{{ urutas.valorPro }}</th>
                                    <th>{{ urutas.inicioPro + ' - ' + urutas.finPro }}</th>
                                    <td v-for="(uproyecciones, indexk, nrok) in urutas.valores" class="oact">
                                        <h5 v-if="uproyecciones !== '-'">{{ urutas.valorPro +'/'+ uproyecciones }}</h5>
                                    </td>
                                    <th>{{ urutas.nombrePro }}</th>
                                </tr>
                            </tbody>
...
export default { 
    ...
    methods: { 
        ...
        Listar() 
        {
            ...
            this.$http.get('/LiberAPP/Proyeccion/ListarServicio/' + this.ruta.idRuta + '/' + this.fechaInicio + '/' + this.fechaFin).then( res => { 
                this.showLoading = false; 
                this.proyecciones = res.data.registros;
    ...
    created() {
        ...
        this.ruta = {idRuta:'0', nombreRut:'TODOS'};
        this.Listar();

        this.$http.get('/LiberAPP/Ruta/Select').then(res => {
            this.rutas = res.data;
            this.rutas.unshift({idRuta:'0', nombreRut:'TODOS'});
        }).catch(error => {
