<template><div>
    <b-row>
        <b-col md="3" xl="3">
            <b-card>
                <h4 class="text-primary">ALMACÉN DE MONTAJE</h4>
                <v-select label="nombreEnt" style="flex: 1;" v-model="ymontaje" :options="entidades" @input="Cargar('montaje')" :disabled="showLoading"/>
            </b-card>
            <b-card>
                <h3>{{ymontaje.nombreEnt}}</h3>
                <draggable :list="montajes" tag="ul" group="llantas" class="list-group list-group-flush cursor-move">
                    <b-list-group-item v-for="(uelemento, index) in montajes" :key="index" tag="li">
                        <blockquote class="blockquote pl-1 border-left-primary border-left-3">
                            <b-badge variant="light-primary" class="d-block my-0" style="font-weight: bold !important"><feather-icon icon="DiscIcon" class="mr-25"/> <span>{{uelemento.codigoEle}}</span></b-badge>
                            <small class="text-secondary" style="font-weight: bold !important"> {{uelemento.marca}} - {{uelemento.modelo}} </small>
                            <small>
                                <ul>
                                    <li>Cocada: 25"</li>
                                    <li>Presión: 125 psi</li>
                                </ul>
                            </small>
                            <footer class="blockquote-footer"> Almacén Michellin </footer>
                        </blockquote>
                    </b-list-group-item>
                </draggable>
            </b-card>
        ...
        <b-col md="6" xl="6">
            <b-card>
                <h4 class="text-success">VEHÍCULO</h4>
                <v-select label="nombreEnt" style="flex: 1;" v-model="yunidad" :options="vehiculos" @input="Cargar('unidad')" :disabled="showLoading"/>
            </b-card>
            <b-card no-body class="border-primary" >
                <b-card-header class="d-flex justify-content-between align-items-center pt-75 pb-25">
                    <div class="d-flex align-items-center mr-2">
                        <b-avatar variant="light-success" rounded> <feather-icon icon="TruckIcon" size="18"/></b-avatar>
                        <div class="ml-1">
                            <h4 class="mb-0 text-success">V2S-585</h4>
                            <span class="card-text negrita">FREIGHTLINER - SD 114</span>
                            <small class="text-muted w-100"></small>
                        </div>
                    </div>
                    <b-badge variant="light-primary" class="negrita"> Tracto </b-badge>
                </b-card-header>
                <b-card-body>
                    <table class="mt-2 mt-xl-0 w-100">
                        <tr>
                            <td class="pb-25">
                                <feather-icon icon="ChevronsRightIcon" class="mr-25"/>
                                <span>Kilometraje: </span>
                                <b-form-input size="sm" class="py-1"/>
                            </td>
                            <td class="pb-25">
                                <feather-icon icon="ChevronsRightIcon" class="mr-25"/>
                                <span>Hubodometro: </span>
            ...
            <b-card>
                <table class="table table-condensed table-bordered vehiculo_llanta">
                    <tr class="llanta_fila"><td></td><td></td><td></td><td></td><td></td></tr>
                    <tr v-for="(ufila,index) in montados.filas">
                        <template v-for="(ucolumna,index2) in ufila">
                            <td v-if="ucolumna.est == 'fila' || ucolumna.est == 'columna'"></td>    
                            <td v-else v-bind:colspan="((ufila.length == 5)? 1:((ufila.length == 3) ? 2 : 5 ))" class="llanta">
                                <b-avatar :text="ucolumna.nro" :variant="(ucolumna.ele.length == 1)?'light-success':'light-danger'" style="font-weight: bold !important"/>
                                <!--<draggable :list="list3" tag="ul" group="llantas" class="list-group list-group-flush cursor-move" :move="checkMove">-->
                                <draggable :list="ucolumna.ele" tag="ul" group="llantas" class="list-group list-group-flush cursor-move" :move="Movimiento" @change="Cambio" @end="Final" >
                                    <b-list-group-item v-for="(uelemento, index) in ucolumna.ele" :key="index" tag="li">
                                        <blockquote class="blockquote pl-1 border-left-success border-left-3 text-justify">
                                            <b-badge variant="light-success" class="d-block my-0" style="font-weight: bold !important"><feather-icon icon="DiscIcon" class="mr-25"/> <span>{{uelemento.codigoEle}}</span></b-badge>
                                            <small class="text-secondary" style="font-weight: bold !important"> {{uelemento.marca}} - {{uelemento.modelo}} </small>
                                            <small>
                                                <ul>
                                                    <li>Cocada: 25"</li>
        ...
        <b-col md="3" xl="3">
            <b-card>
                <h4 class="text-warning">ALMACÉN DE DESMONTAJE</h4>
                <v-select label="nombreEnt" style="flex: 1;" v-model="ydesmontaje" :options="entidades" @input="Cargar('desmontaje')" :disabled="showLoading"/>
            </b-card>
            <b-card>
                <h3>{{ydesmontaje.nombreEnt}}</h3>
                <draggable :list="desmontajes" tag="ul" group="llantas" class="list-group list-group-flush cursor-move">
                    <b-list-group-item v-for="(uelemento, index) in desmontajes" :key="index" tag="li">
                        <blockquote class="blockquote pl-1 border-left-warning border-left-3">
                            <b-badge variant="light-warning" class="d-block my-0" style="font-weight: bold !important"><feather-icon icon="DiscIcon" class="mr-25"/> <span>{{uelemento.codigoEle}}</span></b-badge>
                            <small class="text-secondary" style="font-weight: bold !important"> {{uelemento.marca}} - {{uelemento.modelo}} </small>
                            <small>
                                <ul>
                                    <li>Cocada: 25"</li>
    ...
    <b-card>
        ...
        <!--<b-row>
            <b-col md="12" xl="12" class="text-center">
                ...
                <b-button variant="warning" :href="'/LiberAPP/Kilometraje/Exportar/' + this.ycuenta + '/'+ this.yvehiculo + '/'+ this.tipo + '/' + this.yfinicio + '/' + this.yffin" target="_blank">
    ...
    <!---------------------------------- formulario -------------------------------->
        <b-row>
            <b-col md="12" xl="6">
                <b-form-group label="Cocada Izquierda">
                    <b-form-input v-model="cambio.codigoEle"/>
                </b-form-group>
            </b-col>
            <b-col md="12" xl="6">
                <b-form-group label="Cocada Derecha">
                    <b-form-input v-model="cambio.codigoEle"/>
                </b-form-group>
            </b-col>
            <b-col md="12" xl="6">
                <b-form-group label="Presión">
                    <b-form-input v-model="cambio.codigoEle"/>
                </b-form-group>
            </b-col>
            <b-col md="12" xl="6">
                <b-form-group label="Torque">
                    <b-form-input v-model="cambio.codigoEle"/>
                </b-form-group>
            </b-col>
        </b-row>
        ...
        <template #modal-footer>
            <div class="d-flex justify-content-between">
                <b-button variant="success" size="sm" @click="Actualizar">
...
export default {
    ...
    methods : {
        ...
        Cargar(prm_tipo)
        {
            ...
            this.$http.get('/LiberAPP/Entidad/ListarElementos/'+prm_tipo+'/'+entidad).then(res => {
                this.showLoading = false;
                switch(prm_tipo)
                {
                    case 'montaje':
                        this.montajes = res.data;
                        break;
                    case 'desmontaje':
                        this.desmontajes = res.data;
                        break;
                    case 'unidad':
                        this.montados = res.data;
                        break;
                }
    ...
    created() {
        ...
        this.$http.get('/LiberAPP/Entidad/Listar/TERCERO-ALMACEN').then(res => {
        ...
        this.$http.get('/LiberAPP/Entidad/Listar/VEHICULO').then(res => {
...
<style scoped>
    .vehiculo_llanta td, .vehiculo_llanta th, .vehiculo_llanta
    {
        border: none;
    }
    .llanta_fila td:nth-child(1),.llanta_fila td:nth-child(2),.llanta_fila td:nth-child(4),.llanta_fila td:nth-child(5)
    {
        width: 25%;
    }
    .llanta
