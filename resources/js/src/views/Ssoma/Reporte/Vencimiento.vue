<template>
    <b-row>
        <b-col>
            <ssoma-vencimiento v-if="entidades[0] !== undefined && areas[0] !== undefined"
...
export default { 
    ...
    { 
        ...
        this.$http.get('/LiberAPP/Dato/Select/ENTIDAD/GENERAL').then(res=> {
            this.entidades = res.data.filter( function ( entidad ) { return entidad.valorDat == 'PERSONA' });
        }).catch(error => { 
        ...
        this.$http.get('/LiberAPP/Area/Select').then(res=> {
            this.areas = res.data.filter( function ( area ) { return area.nombreAre == 'SSOMA' });
        }).catch(error => { 
