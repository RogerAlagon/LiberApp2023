                    ...
                    <table class="table table-condensed table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th rowspan="2">Labo rado</th>
                                <th rowspan="2">Para do</th>
                                <th rowspan="2">Man tto</th>
                                <th rowspan="2">Total Días</th>
                                <th rowspan="2">Acum ulado</th>
                                <th rowspan="2">% Ocu pacion</th>
                                <th rowspan="2">% Rendi miento</th>
                            </tr>
                            <tr>
                        ...
                        <tbody>
                            <tr v-for="(seguimiento,indexp) in seguimientos">
                                <th>{{ seguimiento.nro }}</th>
                                <th>{{ seguimiento.pla }}</th>
                                <td v-for="(dia,indexa,nro) in seguimiento.asi" v-if="nro+1 >= adia && dia.asi == 'VI'"  class="oact" @click="Seleccionar(dia.id,indexp,indexa,$event)">
                                        <span class="label label-sm btn-dark-green asistencia" v-bind:class="[colores[dia.asi]]" v-tooltip="dia.com">{{ dia.asi }}</span>
                                        <!--<span class="label label-sm btn-primary ohora" v-if="dia.pre != '' && dia.pre != null">S/.{{ dia.pre }}</span>-->
                                </td>
                                <td v-else-if="nro+1 >= adia && dia.asi != ''" class="oact">
                                        <span class="label label-sm btn-dark-green" v-bind:class="[colores[dia.asi]]" v-tooltip="dia.com">{{ dia.asi }}</span>
                                        <!--<span class="label label-sm btn-primary ohora" v-if="dia.pre != '' && dia.pre != null">S/.{{ dia.pre }}</span>-->
                                </td>
                                <td v-else-if="nro+1 >= adia"  class="oact" @click="Seleccionar(dia.id,indexp,indexa,$event)">
                                    <small>
                                        <span class="omini">{{ nro+1 }}</span>
                                    </small>
                                </td>
                                <td v-else class="oina">
                                    <small>
                                        <span class="omini">{{ nro+1 }}</span>
                                    </small>
                                </td>
                                <td><span class="text-success text-bold">{{ seguimiento.vi }}</span></td>
                                <td><span class="text-danger text-bold">{{ seguimiento.man }}</span></td>
                                <td><span class="text-warning text-bold">{{ seguimiento.par }}</span></td>
                                <td><span class="text-warning text-bold">{{ seguimiento.com }}</span></td>
                                <td><span class="text-primary text-bold">{{ seguimiento.vi + seguimiento.man + seguimiento.par + seguimiento.com}}</span></td>
                                <td><span class="text-primary text-bold">% {{ seguimiento.ocu}}</span></td>
                                <td><span class="text-primary text-bold">% {{ seguimiento.ren}}</span></td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr>
...
export default {
    ...
    methods : {
        ...
        Listar()
        {
            ...
            this.$http.get('/LiberAPP/Seguimiento/Periodo/'+this.fecha+'/'+this.tipo+'/TODO')
            .then(res => {
                this.showLoading = false;
                this.dias = res.data.dias;
                this.seguimientos = res.data.seguimientos;
                //this.adia = res.data.adia

                //rellenar dias del mes
                var acumulador = res.data.dia_semana;
                this.dias_mes = [];
                for (var udia in this.dias)
                {
                    this.dias_mes.push(this.dias_semana[acumulador]);
                    acumulador++;
                    if(acumulador == 7)
                        acumulador = 0;
                }
                //$(".table tr td").removeClass("osombreado");
                this.seleccionados = [];
