<template><div>
            ...
            <b-row>
                <b-col md="6" xl="3">
                    <b-form-group label="Plantilla:">
                        <v-select label="nombrePla" :options="plantillas" v-model="plantilla" style="flex: 1;"/>
                    </b-form-group>
                </b-col>
                <b-col md="6" xl="3">
                    <b-form-group label="Eje X:">
                        <!--s<v-select label="valorDat" :options="zentidades" v-model="entidad" style="flex: 1;"/>-->
                        <b-form-select v-model="ejex" :options="['VEHICULO','MANTENIMIENTO']" :select-size="2"/>
                    </b-form-group>
                </b-col>
                <b-col md="6" xl="3">
                    <b-form-group label="Eje Y:">
                        <!--s<v-select label="valorDat" :options="zentidades" v-model="entidad" style="flex: 1;"/>-->
                        <b-form-select v-model="ejey" :options="['DIAS','SEMANAS','MESES']" :select-size="3"/>
                    </b-form-group>
                </b-col>
                <b-col md="6" xl="3">
                    <b-form-group label="Proyección:">
                        <b-form-select v-model="proyeccion" :options="['ANTICIPADO','REAL']" :select-size="2"/>
                        <!--<v-select label="valorDat" :options="zentidades" v-model="entidad" style="flex: 1;"/>-->
                    </b-form-group>
                </b-col>
                <b-col md="6" xl="6" class="text-center">
                    <div class="demo-inline-spacing">
                        <b-button variant="primary" @click="Listar()" :disabled="showLoading">
                            <feather-icon icon="ArrowDownIcon" class="mr-50" v-if="!showLoading"/>
                            <b-spinner small v-else/>
                            <span class="align-middle">Proyectar</span>
                        </b-button>
                        <b-button variant="warning" :href="'/LiberAPP/Asignacion/ExportarProyeccion/'+ this.idEntidad+'/'+ this.idArea +'/44/ENTIDAD/DIAS/REAL'" 
                          target="_blank">
    ...
    <b-card>
        <b-row>
            <b-col md="12" xl="12">
                <div class="otabla">
                    <table class="table table-condensed table-bordered table-hover table-hover">
                        <!--<tr><th colspan="11" class="titulo"></th></tr>-->
                        <thead>
                            <tr>
                                <th rowspan="2">NRO</th>
                                <th rowspan="2">VEHICULO</th>
                                <template v-if="x == 'ENTIDAD'">
                                    <th rowspan="2">KM. ACTUAL</th>
                                    <th rowspan="2">KM. PROMEDIO</th>
                                </template>
                                <th v-for="(uy, indice) in arr_ejey">{{uy.i}}</th>
                            </tr>
                            <tr>
                                <th v-for="(uy, indice) in arr_ejey">{{uy.f}}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(ux, indice,nro) in arr_ejex">
                                <th class="subsubtitulo">{{nro+1}}</th>
                                <th class="subsubtitulo"><a style="color: white !important" :href="'/LiberAPP/Asignacion/Historial/'+ idEntidad+'/'+idArea+'/'+plantilla.idPlantilla+'/VEHICULO/'+ux.id" target="_blank">{{ux.nom}}</a></th>
                                <template v-if="x == 'ENTIDAD'">
                                    <td>{{ux.act}}</td>
                                    <td>{{ux.pro}}</td>
                                </template>
                                <td v-for="(proy, indice, pos) in ux.y" :class="[(pos == 0 && proy.length > 0) ? 'cell-danger':'', (pos == 1 && proy.length > 0) ? 'cell-warning':'']">
                                    <template v-if="proy.length > 1">
                                        <button v-b-modal.modal-formulario @click="Mostrar(proy)" size="sm" v-if="pos == 0" class="btn btn-sm" :class="colores[proy[proy.length-1].nom]">{{proy[proy.length-1].nom}}</button>
                                        <button v-b-modal.modal-formulario @click="Mostrar(proy)" size="sm" v-else-if="pos == 1" class="btn btn-sm" :class="colores[proy[proy.length-1].nom]">{{proy[proy.length-1].nom}}</button>
                                        <button v-b-modal.modal-formulario @click="Mostrar(proy)" size="sm" v-else class="btn btn-sm" :class="colores[proy[proy.length-1].nom]">{{proy[proy.length-1].nom}}</button>
                                    </template>
                                    <template v-else-if="proy.length == 1">
                                        <button v-b-modal.modal-formulario @click="Mostrar(proy)" size="sm" class="btn btn-sm" v-if="pos == 0" :class="colores[proy[0].nom]">{{proy[0].nom}}</button>
                                        <button v-b-modal.modal-formulario @click="Mostrar(proy)" size="sm" class="btn btn-sm" v-else-if="pos == 1" :class="colores[proy[0].nom]">{{proy[0].nom}}</button>
                                        <button v-b-modal.modal-formulario @click="Mostrar(proy)" size="sm" class="btn btn-sm" v-else :class="colores[proy[0].nom]">{{proy[0].nom}}</button>
                                    </template>
                                    <!--
                                        <b-button v-b-modal.modal-formulario @click="Mostrar(proy)" size="sm" variant="outline-danger" v-if="proy.length > 0 && pos == 0">{{proy.length}}</b-button>
                                        <b-button v-b-modal.modal-formulario @click="Mostrar(proy)" size="sm" variant="outline-warning" v-else-if="proy.length > 0 && pos == 1">{{proy.length}}</b-button>
                                        <b-button v-b-modal.modal-formulario @click="Mostrar(proy)" size="sm" variant="outline-success" v-else-if="proy.length > 0">{{proy.length}}</b-button>
                                    -->
                                </td>
                            </tr>
                            <tr v-for="(ux, indice,nro) in arr_ejex">
                                <th class="subsubtitulo">-</th>
                                <th class="subsubtitulo"><a style="color: white !important" :href="'/LiberAPP/Asignacion/Historial/'+ idEntidad+'/'+idArea+'/'+plantilla.idPlantilla+'/VEHICULO/'+ux.id" target="_blank">{{ux.nom}}</a></th>
                                <template v-if="x == 'ENTIDAD'">
                                    <td>{{ux.act}}</td>
                                    <td>{{ux.pro}}</td>
...
export default {
    ...
    methods : {
        ...
        Listar()
        {
            ...
            //this.area.idArea = 4;
            this.showLoading = true;
            if(this.ejex == "VEHICULO")
                this.ejex = "ENTIDAD";
            else
                this.ejex = "IMPLEMENTO";
            this.x = this.ejex;



            this.$http.get('/LiberAPP/Asignacion/Proyeccion/'+ this.idEntidad+'/'+this.idArea+'/'+this.plantilla.idPlantilla+'/'+this.ejex+'/'+this.ejey+'/'+this.proyeccion).then(res => {

                this.showLoading = false;
                
                this.arr_ejey = res.data.arr_ejey;
                this.arr_ejex = res.data.arr_ejex;
                this.seleccionados = [];
