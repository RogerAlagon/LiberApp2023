    ...
    <b-card>
        <b-row>
            ...
            <b-col md="6" xl="3">
                <b-form-group label="Vehiculo">
                    ...
                    <!--<v-select label="placaVeh" v-model="yvehiculo" :options="yvehiculos" :reduce="vehiculo => vehiculo.idVehiculo" style="flex: 1;"/>-->
                </b-form-group>
            </b-col>
            <b-col md="6" xl="2">
            ...
            <!--<b-col md="6" xl="3">
                <h6>Mes-Año:</h6>
                <b-form-datepicker v-model="fecha" class="form-control" :date-format-options="{ year: 'numeric', month: 'numeric', day: 'numeric' }" locale="es" new=date()>
                </b-form-datepicker>
            </b-col>-->
            <!--<b-col md="6" xl="3">
                <b-form-group label="Tipo Lista:">
        ...
        <b-row>
            <b-col md="12" xl="12" class="text-center">
                ...
                <b-button variant="warning" :href="'/LiberAPP/Kilometraje/Exportar/' + this.ycuenta.idDato + '/'+ this.yvehiculo.idVehiculo + '/'+ this.tipo + '/' + this.yfinicio + '/' + this.yffin" target="_blank">
    ...
    <b-card>
      <b-row>
        <div class="d-flex justify-content-start flex-wrap">
          <div class="text-center combustible colors-container rounded text-white width-100 height-50 d-flex align-items-center justify-content-center mr-1 ml-50 my-1 shadow">
            <span>RRCC</span>
          </div>
          <div class="text-center movil colors-container rounded text-white width-100 height-50 d-flex align-items-center justify-content-center mr-1 ml-50 my-1 shadow">
            <span>ESCOLTA</span>
          </div>
        </div>
      </b-row>
        <b-row>
            <b-col md="12" xl="12" v-if="seleccionado.tipo == 'REGISTRO'">
                <div class="otabla">
                    <table class="table table-condensed table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th rowspan="2">9</th>
                                <th rowspan="2">10</th>
                                <!--<th v-for="dia in dias_mes">{{ dia }}</th>-->
                                <th rowspan="2">Vehículo</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(uvehiculo,indexv,nrov) in kilometrajes">
                                <th>{{ nrov+1 }}</th>
                                <th>{{ uvehiculo.placaVeh }}</th>
                                <td>{{ uvehiculo.marca }}</td>
                                <td>{{ uvehiculo.modelo }}</td> 
                                <td>{{ uvehiculo.tipo }}</td>
                                <td v-for="n in (10 - uvehiculo.kilometrajes.length)" class="oact">
                                        <!--<span class="label label-sm btn-dark-green asistencia" v-bind:class="[colores[dia.asi]]" v-tooltip="dia.com">{{ dia.asi }}</span>-->
                                        -
                                </td>
                                <!--<td v-for="(ukilometraje,indexk,nrok) in uvehiculo.kilometrajes" class="oact" @click="Seleccionar(dia.id,indexp,indexa,$event)" :class="{ 'bg-danger': ukilometraje.error }">-->
                                <td v-for="(ukilometraje,indexk,nrok) in uvehiculo.kilometrajes" class="oact" @click="Seleccionar(uvehiculo.idVehiculo, ukilometraje.fechaKil, ukilometraje.kilometrajeKil, 'registro', ukilometraje.idKilometraje, uvehiculo)" v-b-modal.modal-formulario :class="{ 'bg-danger': ukilometraje.error }">
                                        <!--<span class="label label-sm btn-dark-green asistencia" v-bind:class="[colores[dia.asi]]" v-tooltip="dia.com">{{ dia.asi }}</span>-->
                                        <h5 :class="{ 'text-light': ukilometraje.error }">{{ukilometraje.kilometraje}}</h5>
                                        <small class="text-muted">{{ukilometraje.fechaKil}}</small>
            ...
            <b-col md="12" xl="12" v-else-if="seleccionado.tipo == 'DIA'">
                <div class="otabla">
                    <table class="table table-condensed table-bordered">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <tr v-for="(uvehiculo,indexv,nrov) in kilometrajes">
                                ...
                                <td>{{ uvehiculo.modelo }}</td> 
                                <td>{{ uvehiculo.tipo }}</td>
                                <!--<td v-for="(ukilometraje,indexk,nrok) in uvehiculo.kilometrajes" class="oact" @click="Seleccionar(dia.id,indexp,indexa,$event)">-->
                                <td v-for="(ukilometraje,indexk,nrok) in uvehiculo.kilometrajes" :class="{ 'movil': ukilometraje.movil, 'combustible': ukilometraje.combustible }" class="oact" @click="Seleccionar(uvehiculo.idVehiculo, indices[indexk], ukilometraje, 'dia', ukilometraje, uvehiculo) " v-b-modal.modal-formulario>
                                        <h4 class="text-bold">{{ukilometraje.kilometrajeKil}}</h4>
                                        <small class="text-dark text-bold">{{ukilometraje.nombreUsu}}</small>
                                        <!--<h5>{{ukilometraje}}</h5>-->
                                        <!--<small class="text-muted">{{ukilometraje.fechaKil}}</small>-->
                                </td>
                                <th>{{ uvehiculo.placaVeh }}</th>
            ...
            <b-row>
              <b-col md="4" xl="6">
                  <b-form-group label="ID">
                        <b-form-input v-model="obj_kilometraje.Vehiculo_idVehiculo" readonly/>
                  </b-form-group>
              </b-col>
              <b-col md="6" xl="6">
                  <b-form-group label="Placa">
                        <b-form-input v-model="obj_kilometraje.placaVeh" readonly/>
                  </b-form-group>
              </b-col>
            </b-row>
            <b-row>
              <b-col md="6" xl="6">
                    <b-form-group label="Fecha Kilometraje">
                        <flat-pickr class="form-control" v-model="obj_kilometraje.fechaKil" :disabled="estadoFecha"/>
                     </b-form-group>
                </b-col>
                <b-col md="6" xl="6">
                    <b-form-group label="Kilometraje">
                        <b-form-input v-model="obj_kilometraje.kilometrajeKil" readonly/>
                    </b-form-group>
                </b-col>
                <b-col md="6" xl="6">
                    <b-form-group label="Sumar Kilometraje">
                        <b-form-input v-model="obj_kilometraje.kilometrajeNuevo" @input="obj_kilometraje.kilometrajeTotal = parseInt(obj_kilometraje.kilometrajeNuevo) + parseInt(obj_kilometraje.kilometrajeKil)"/>
                    </b-form-group>
                </b-col>
                <b-col md="6" xl="6">
                    <b-form-group label="Kilometraje Nuevo">
                        <b-form-input v-model="obj_kilometraje.kilometrajeTotal" readonly/>
                    </b-form-group>
                </b-col>
                <b-col md="6" xl="12">
                    <b-form-group label="Aplicar Km al Aplicativo Móvil">
                      <b-button v-if="obj_kilometraje.seleccionado == false" variant="danger" size="sm" @click="obj_kilometraje.seleccionado=true" block>DESACTIVADO</b-button>
                      <b-button v-if="obj_kilometraje.seleccionado == true" variant="success" size="sm" @click="obj_kilometraje.seleccionado=false" block>ACTIVADO</b-button>
                    </b-form-group>
                </b-col>
            </b-row>
            <!---------------------------------- modal acciones -------------------------------->
            <template #modal-footer>
                <div class="w-100 d-flex justify-content-between">
                    ...
                    <b-button variant="success" size="sm" @click="GuardarKm" :disabled="showLoading">
                        <feather-icon icon="CheckIcon" class="mr-50" v-if="!showLoading"/>
                        <b-spinner small v-else/>
                        <span class="align-middle">Guardar</span>
                    </b-button>
                    <b-button variant="danger" size="sm" :disabled="showLoading" v-if="obj_kilometraje.eliminar == true" @click="Eliminar(obj_kilometraje.idKilometraje)">
                        <feather-icon icon="CheckIcon" class="mr-50" v-if="!showLoading"/>
                        <b-spinner small v-else/>
                        <span class="align-middle">Deshabilitar</span>
                    </b-button>
                </div>
            </template>
