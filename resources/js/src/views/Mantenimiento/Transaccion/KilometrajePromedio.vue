<template>
    <div>
        <b-card>
            <!--<b-row>
                <b-col md="6" xl="4">
                    <b-form-group label="Vehiculo">
                        <v-select label="placaVeh" :options="vehiculos" :reduce="vehiculo => vehiculo.idVehiculo" style="flex: 1;"/>
                    </b-form-group>
                </b-col>
                <b-col md="6" xl="4">
                    <b-button v-b-modal.modal-material variant="gradient-success">
                        <feather-icon icon="PlusIcon" class="mr-50"/>
                        <span class="align-middle">CALCULAR KM PROMEDIO</span>
                    </b-button>
                </b-col>
            </b-row>-->
            ...
            <b-row>
                <b-col md="6" xl="4">
                    <b-form-group label="Seleccione Rango (días)">
                        <!--<v-select label="placaVeh" v-model="idVehiculo" :options="vehiculos" :reduce="vehiculo => vehiculo.idVehiculo" style="flex: 1;"/>-->
                        <b-form-select v-model="ydia" :options="dias"/>
                    </b-form-group>
                </b-col>
        ...
        <b-card>
            <b-row class="custom-search d-flex justify-content-end">
                <div>
                    <b-button v-b-modal.modal-material variant="gradient-success" @click="GuardarPromedio" :disabled="showLoading">
                        <feather-icon icon="PlusIcon" class="mr-50" v-if="!showLoading"/>
                        <b-spinner small v-else/>
                        <span class="align-middle">GUARDAR PROMEDIO</span>
                    </b-button>
                </div>
            </b-row>
            <br>
                        ...
                        <table class="table table-condensed table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th rowspan="2">Ultimo Km</th>
                                    <th rowspan="2">Km Promedio</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(uvehiculo, indexv, nrov) in promedios">
                                    <th>{{ uvehiculo.idVehiculo }}</th>
                                    <th>{{ uvehiculo.vehiculo }}</th>
                                    <td>{{ uvehiculo.ultimokm }}</td>
                                    <td>{{ uvehiculo.promedio }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
export default { 
    ...
    methods: { 
        ...
        Listar()
        {
            ...
            this.$http.get('/LiberAPP/Kilometraje/KilometrajePromedio/'+ this.ydia).then(res=> {
                this.promedios = res.data.registros;
                this.showLoading = false;
            }).catch(error => {
                this.Toast(true,'danger','Clasificacion','Error al cargar los registros: ' + error);
                this.showLoading = false;
            })
        },
        GuardarPromedio() 
        {
            var jsonPromedios = JSON.stringify( this.promedios )
            this.showLoading = true;
            this.$http.post('/LiberAPP/Vehiculo/GuardarPromedioKm',
            {
                promedios: jsonPromedios,
            }).then( res => {
                this.Toast(true,'primary','Actualizar km Promedio','Actualizado Correctamente');
                this.showModal = false; 
                this.showLoading = false;
            }).catch( error => {
                this.Toast(true,'danger','km Promedio','Error al guardar los registros: ' + error )
                this.showLoading = false;
            })
        },
    },
    ...
    created() { 
        ...
        /*this.$http.get('/LiberAPP/Kilometraje/KilometrajePromedio/').then(res=> {
            this.promedios = res.data.registros;
        }).catch(error => {
            this.Toast(true,'danger','Clasificacion','Error al cargar los registros: ' + error);
        })*/
